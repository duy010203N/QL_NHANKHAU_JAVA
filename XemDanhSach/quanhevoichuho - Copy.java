...
@WebServlet("/quanhevoichuho")
public class quanhevoichuho extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            String searchMQH = req.getParameter("searchma_quan_he");
...
out.println("</head>");
out.println("<body>");
out.println("<h2 style='color: #fff; background: linear-gradient(to right, #4CAF50, #336699); padding: 15px; border-radius: 8px; border: 2px solid #336699; text-align: center;'>QUAN HỆ VỚI CHỦ HỘ</h2>");
...
out.println("<h2>Tìm kiếm quan hệ với chủ hộ</h2>");
out.println("<form method='get'>");
out.println(" <label for='searchma_quan_he'>Mã Quan Hệ:</label>");
out.println(" <input type='text' id='searchma_quan_he' name='searchma_quan_he'>");
...
out.println("<h3>Thêm mới quan hệ với chủ hộ</h3>");
out.println("<form method='post' action='AddQHRecordServlet'>");
out.println("Mã quan hệ: <input type='text' name='ma_quan_he'><br>");
out.println("Tên quan hệ: <input type='text' name='ten_quan_he'><br>");
out.println("<input type='submit' value='Thêm Mới'>");
out.println("<form method='post' action='AddQHRecordServlet?table=quan_he_voi_chu_ho'>");
out.println("</form>");
out.println("</div>");
...
out.println("</script>");
    ...
    if (tableName == null || tableName.isEmpty() || tableName.equals("ho_khau")) {
        ...
        String sql1 = "SELECT * FROM quan_he_voi_chu_ho";
        String sql2 = "SELECT * FROM quan_he_voi_chu_ho WHERE ma_quan_he LIKE '%" + searchMQH + "%'";
        
        // Use sql2 if searchMQH is not null and not empty
        ResultSet rs;
        if (searchMQH != null && !searchMQH.isEmpty()) {
        ...
        out.println("<tr><th>Mã quan hệ</th><th>Tên quan hệ</th></tr>");

        while (rs.next()) {
            String MQH = rs.getString("ma_quan_he");
            String TQH = rs.getString("ten_quan_he");

            out.println("<tr><td>" + MQH + "</td><td>" + TQH + "</td><td><a href='EditQHRecordServlet?ma_quan_he=" + MQH + "'>Edit</a></td>"
                    + "<td><a href='DeleteQHRecordServlet?ma_quan_he=" + MQH + "'>Delete</a></td></tr>");
        }
    }
} finally {
                if (stmt != null) {
